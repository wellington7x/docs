...
Nesse exemplo, salvamos o valor total do pedido na variável `totalPrice`, acessando o array `orders` e o dado `total_price` (valor total do pedido). Dessa forma, conseguimos executar, por exemplo, um cálculo para aplicar automaticamente um cupom de desconto ao valor total do pedido:
